...
# ISSUE from tensorflow.keras.utils import multi_gpu_model
...
def get_feat_df(df,aa_comp={},num_cores=False,ignore_mods=False):
...
def get_feat_matrix(df):
    ...
    return X, X_sum, X_global, X_hc, np.array(df["tr"])
...
def write_preds(df,
    ...
    for mod in mods:
        if fit_hc:
            pred_test = mod.predict([X,X_sum,X_global,X_hc]).flatten()*correction_factor
        else:
